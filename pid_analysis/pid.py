...
TOTAL_PREDURATION = (
    2  # The total time before a trial will almost always be 2s before odor measurement
)
...
def process_file(file_container):
    ...
    for i in trange(num_trials):
        ...
        baseline_data_baseline_shift = np.subtract(
            baseline_data, avg_baseline_data
        )  # Yes, mostly zeros
        ...
        if trial_type.lower() == "solvent":
            ...
            solvent1_data_baseline_shift_trimmed = solvent1_data_baseline_shift[
                solvent1_duration_s[0] :
            ]

            trial_len = (
                trial_len + len(solvent1_data_baseline_shift) + solvent2_data_len
            )
        ...
        if trial_type.lower() == "solvent":
            row_data = np.hstack(
                (
                    row_data,
                    average_solvent1_response,
                    peak_solvent1_response,
                    average_solvent2_p1_response,
                    peak_solvent2_p1_response,
                    average_solvent2_p2_response,
                    peak_solvent2_p2_response,
                )
            )
    ...
    if trial_type.lower() == "solvent":
        columns = [
            "PID Peak",
            "PID Avg",
            "Solvent(pre odor) Peak",
            "Solvent(pre odor) Avg",
            "Solvent(post odor) 1 Peak",
            "Solvent(post odor) 1 Avg",
            "Solvent(post odor) 2 Peak",
            "Solvent(post odor) 2 Avg",
        ]
    else:
